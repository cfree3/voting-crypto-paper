...
% title
\title{Cryptographic assurance of verifiability and secrecy in voting}
\date{}
\author{
  \begin{tabular}{c c c}
    Kelsey Francis &
...
\begin{onecolabstract}
  ...
  existing voting procedures to provide fairness guarantees while maintaining ballot secrecy. We
	detail security properties necessary to make an election both \term{verifiable} and \term{secret}
	and analyze how three popular verifiable voting systems
	-- \term{Scantegrity}, \term{\pv{}}, and \term{Helios} --
...
\subsection{After voting}
A publicly-available bulletin board publishes all recorded votes, i.e., the right-hand side of each cast ballot, including its onion and vote as recorded.
...
Voters must rely on varied auditing authorities to verify this tallying process.
...
But in \term{\pv{}} and \term{Helios}, use of a cryptographic mixnet means that both inputs and outputs can be mapped to
voters and tallies, respectively, without revealing the inner mapping. While accomplishing the same
goals as \term{Scantegrity}'s Table system, this approach is in some ways its antithesis.
...
While each system provides the voter some assurance that the proper vote was recorded, \term{Scantegrity}'s
...
\term{Scantegrity} resists coercion via the manner it uses to encrypt ballots. Firstly, assuming
physical security, without access to Table P or Table R, one cannot predict the confirmation codes
that will be produced by casting a ballot in a particular manner. Secondly, without exposure of the
data maintained in Table P or Table R, a voter has no way to prove to a third party that she has
voted as claimed.

An important point regarding \term{Scantegrity} is that it also relies on a \emph{physical property} -- the
invisible ink used when printing confirmation code bubbles -- to combat vote coercion. This is akin
to keeping Table P a secret: the ballots together \emph{are} Table P, and using special ink allows
the codes to be printed without making it easy for a ballot handler to simple read Table P from the
...
information necessary to \emph{prove} a decryption of her ballot, thus providing receipt-freeness. Though a voter is able to see
